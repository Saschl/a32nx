...
class Guidance {
    /**
     * Creates an instance of the Guidance.
     ...
     * @param {GuidanceOptions} options The LNAV options to use with this instance.
     */
    constructor(fpm, navModeSelector, options) {
       ...
       */
        ...
        this.options = options || new GuidanceOptions();
    ...
    update() {
        ...
        if (this.activeFlightPlan) {
            ...
            const planeState = Guidance.getAircraftState();
    ...
    generateGuidance(activeWaypoint, planeState, previousWaypoint, navSensitivity, navSensitivityScalar) {
        ...
        const trackAngleError = mod(dtk - planeState.trueTrack + 180, 360) - 180;

        if (trackAngleError !== this.lastTAE) {
            console.log("TAE " + trackAngleError);
        ...
        /*    switch (this.state) {
              ...
              && (activeMode === LateralNavModeState.LNAV || (activeMode === LateralNavModeState.APPR && this.navModeSelector.approachMode === WT_ApproachType.RNAV));
                    Guidance.trackLeg(previousWaypoint.infos.coordinates, activeWaypoint.infos.coordinates, planeState, navSensitivity, navSensitivityScalar, shouldExecute);
                    break;
    ...
    static trackLeg(legStart, legEnd, planeState, navSensitivity, navSensitivityScalar = 1, execute = true) {
        ...
        const fullDeflection = Guidance.getFullDeflectionValue(navSensitivity, navSensitivityScalar);
        ...
        if (execute) {
            Guidance.setCourse(headingToSet, planeState);
        }
    }
    ...
    handleTurnCompleting(planeState, dtk, previousWaypoint, activeWaypoint, navSensitivity, navSensitivityScalar) {
        const angleDiffToTarget = Math.abs(Avionics.Utils.diffAngle(planeState.trueHeading, dtk));

        const trackAngleError = mod(desiredTrack - trueTrack + 180, 360) - 180;

        if (angleDiffToTarget !== this.lastTAE) {
            console.log("ANGLE",angleDiffToTarget);

            //SimVar.SetSimVarValue('L:A32NX_FG_TRACK_ANGLE_ERROR', 'degree', angleDiffToTarget);
            this.lastTAE = angleDiffToTarget;
        }
        /*    if (Math.abs(angleDiffToTarget) < this.options.degreesRollout || this.navModeSelector.currentLateralActiveState !== LateralNavModeState.LNAV) {
            ...
            Guidance.trackLeg(previousWaypoint.infos.coordinates, activeWaypoint.infos.coordinates, planeState, navSensitivity, navSensitivityScalar, false);
            Guidance.setCourse(targetHeading, planeState);
        } */
    }
    ...
    sequenceToNextWaypoint(planeState, currentWaypoint) {
        if (this.sequencingMode !== FlightPlanSequencing.INHIBIT && planeState.groundSpeed > 25 && !planeState.onGround) {
            ...
            if (currentWaypoint && currentWaypoint.endsInDiscontinuity) {
                ...
                //   Guidance.setCourse(SimVar.GetSimVarValue('PLANE HEADING DEGREES TRUE', 'Radians') * Avionics.Utils.RAD2DEG, planeState);
                SimVar.SetSimVarValue('L:WT_CJ4_WPT_ALERT', 'number', 0);
    ...
    tryActivateIfArmed(legStart, legEnd, planeState, navSensitivity) {
        ...
        const xtk = Math.abs(AutopilotMath.crossTrack(legStart, legEnd, planeState.position));
        ...
        if (xtk !== this.lastXTE) {
            this.lastXTE = xtk;
            console.log("XTk " + xtk);
            SimVar.SetSimVarValue('L:A32NX_FG_CROSS_TRACK_ERROR', 'nautical miles', xtk);
        }
...
class GuidanceOptions {
    ...
    /**
     * Creates an instance of GuidanceOptions.
     */
    constructor() {
        /**
...
class LatLon {
    ...
    static intersection(p1, brng1, p2, brng2) {
        if (!(p1 instanceof LatLon)) {
            p1 = LatLon.parse(p1);
        } // allow literal forms
        if (!(p2 instanceof LatLon)) {
            p2 = LatLon.parse(p2);
        } // allow literal forms
        if (isNaN(brng1)) {
        ...
        if (Math.abs(δ12) < Number.EPSILON) {
            return new LatLon(p1.lat, p1.lon);
        } // coincident points
    ...
    crossTrackDistanceTo(pathStart, pathEnd, radius = 6371e3) {
        if (!(pathStart instanceof LatLon)) {
            pathStart = LatLon.parse(pathStart);
        } // allow literal forms
        if (!(pathEnd instanceof LatLon)) {
            pathEnd = LatLon.parse(pathEnd);
        } // allow literal forms
        const R = radius;
