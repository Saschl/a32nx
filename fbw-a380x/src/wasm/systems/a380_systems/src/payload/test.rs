...
fn backend_target_cargo_distribution_command_updates_station_targets() {
    let mut test_bed = test_bed_with()
        .init_vars()
        .and_run()
        .request_target_cargo_kg(10000.)
        .and_run();

    let mut target_total_kg = 0.;
    for cs in 0..A380Payload::A380_CARGO.len() {
        let target_kg: f64 =
            test_bed.read_by_name(&format!("{}_DESIRED", A380Payload::A380_CARGO[cs].cargo_id));
        target_total_kg += target_kg;
    }

    assert!((target_total_kg - 10000.).abs() <= 1.0);
}
