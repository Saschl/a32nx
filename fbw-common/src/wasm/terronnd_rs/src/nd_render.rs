...
/// Paints RGBA rows `[y_start, y_end)` from the per-block bands: one pattern
/// byte and one table lookup per pixel. (Upstream paints the whole frame in
/// one call; the row range is the local band-slicing deviation.)
///
...
fn paint_rows(
    ...
    for y in y_start..y_end {
        let frame_row = &mut frame[y * width * 4..(y + 1) * width * 4];
...
/// Per-cycle render state: block maxima classified into colour bands plus the
/// per-band pattern lookup tables — everything painting needs, precomputed
/// once at analyze time so the runner can slice the painting across frames.
pub struct PreparedRender {
    bands: Vec<Band>,
    table: [BandColors; BANDS.len()],
    blocks_x: usize,
}

impl PreparedRender {
    pub fn new(elevations: &[i16], geometry: &NdMapGeometry, stats: &RenderStats) -> Self {
        ...
        Self::from_blocks(&maxima, geometry, stats)
    }

    /// Classification straight from precomputed block maxima (the block-native
    /// extractor's output; also `ELEV_INVALID` for arc-cut blocks, which
    /// classifies to the black band like the `BLOCK_EMPTY` seed does).
    pub fn from_blocks(maxima: &[i16], geometry: &NdMapGeometry, stats: &RenderStats) -> Self {
        let blocks_x = geometry.width.div_ceil(8);
        debug_assert_eq!(maxima.len(), blocks_x * geometry.height.div_ceil(8));
    ...
    /// Paints rows `[y_start, y_end)` of the full-size RGBA `frame`.
    pub fn paint_rows(
        &self,
        pattern: &[u8],
        width: usize,
...
pub fn render_navigation_display(
    ...
    let prepared = PreparedRender::new(elevations, geometry, stats);
