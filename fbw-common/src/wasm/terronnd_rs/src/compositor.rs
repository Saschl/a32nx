...
//!
//! The gauge no longer composes screen frames on the CPU — it draws the
//! per-cycle map images at these layout offsets directly with NanoVG
//! (blit.rs). `compose_screen_frame` is retained as the SimBridge reference;
//! the offset constants below are the layout source for the GPU path.
