...
import { FacilityCache, LoadType } from 'navdata/client/backends/Msfs/FacilityCache';
import { JS_Approach, JS_Runway, ApproachType as MSApproachType } from '../FsTypes';

export class FlightPlanAsoboSync {
    private isReady = false;

    private cruiseFlightLevel: undefined | number = 0;

    private originAirport: string | null = null;

    private destinationAirport: string | null = null;

    private procedureDetails = undefined;

    private enrouteLegs: (SerializedFlightPlanLeg | Discontinuity)[] = [];
    ...
    constructor(private readonly bus: EventBus) {
        this.facilityCache = FacilityCache.getInstance();
    }
    ...
    init(): void {
        ...
        this.subs.push(sub.on('flightPlan.setPerformanceData.cruiseFlightLevel').handle(async (event) => {
            if (event.planIndex === FlightPlanIndex.Active && event.value) {
        ...
        try {
            if (this.isReady) {
                ...
                if (this.originAirport && this.destinationAirport) {
                    ...
                    const originFacility = await this.facilityCache.getFacility(`A      ${this.originAirport.substring(0, 4)} `, LoadType.Airport);
                    let originRw = 0;
                    let departureRw = 0;
                    ...
                    const airportFacility = await this.facilityCache.getFacility(`A      ${this.destinationAirport.substring(0, 4)} `, LoadType.Airport);
    ...
    private findApproachIndexByRunwayAndApproachType(
        approaches: readonly JS_Approach[],
        runway: JS_Runway,
