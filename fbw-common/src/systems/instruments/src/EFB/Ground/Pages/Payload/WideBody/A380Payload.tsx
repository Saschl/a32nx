...
export const A380Payload: React.FC<PayloadProps> = ({
  ...
  const [fwdBagDesired] = useSimVar(`L:${cabinInfo.cargoMap[0].simVar}_DESIRED`, 'Number', 607);
  const [aftBagDesired] = useSimVar(`L:${cabinInfo.cargoMap[1].simVar}_DESIRED`, 'Number', 613);
  const [aftBulkDesired] = useSimVar(`L:${cabinInfo.cargoMap[2].simVar}_DESIRED`, 'Number', 617);
 ...
 */
  ...
  const [zfwDesired] = useSimVar('L:A32NX_WB_TARGET_ZFW_KG', 'Number', 1_909);
  const [gwDesired] = useSimVar('L:A32NX_WB_TARGET_GW_KG', 'Number', 1_911);
  ...
  const setTargetPax = useCallback(
    (numOfPax: number) => {
      ...
      // GSX is an external app polling this LVar directly, so it must stay a SimVar write
      SimVar.SetSimVarValue('L:FSDT_GSX_NUMPASSENGERS', 'Number', clampedPax);
